...
#이미지 사이즈 키우고 graysclae로 변환
img = Image.open(r"C:\Users\alfot\Desktop\ocr_pytesseract\img\text1.jpg")
img_numpy = np.array(img, 'uint8')
gray0 = cv2.cvtColor(img_numpy, cv2.COLOR_BGR2GRAY)
height, width = gray0.shape
gray = cv2.resize(gray0, (2*width, 2*height), interpolation=cv2.INTER_LINEAR)
cv2.imwrite(r"C:\Users\alfot\Desktop\ocr_pytesseract\img\text1Gray.jpg",gray)
print("All Done")
...
def otsu_thresh(src):
    ...
    # 히스토그램 계산
    hist = calc_histo(src)
    # 히스토그램을 정규화 하여 픽셀 값에 대한 등장 확률 계산
...
image_path = r"C:\Users\alfot\Desktop\ocr_pytesseract\img\text1Gray.jpg"
...
cv2.imwrite(r"C:\Users\alfot\Desktop\ocr_pytesseract\img\imageotsu.jpg",img_otsu)

# 테서랙트로 이미지에서 텍스트 추출
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
text = pytesseract.image_to_string(Image.open(r"C:\Users\alfot\Desktop\ocr_pytesseract\img\imageotsu.jpg"), lang="kor")
print(text)
# text = re.sub('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>\{\}`\'…》]', '', text)
# print(text.replace(" ", ""))
